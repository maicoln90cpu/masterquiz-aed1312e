...
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS is_icp_profile boolean DEFAULT NULL;

COMMENT ON COLUMN public.profiles.is_icp_profile IS
  'Persona ICP: TRUE=ON (comercial), FALSE=OFF (educacional), NULL=não selecionou.';

-- 2) Backfill
UPDATE public.profiles SET is_icp_profile = CASE
  WHEN user_objectives && ARRAY['paid_traffic','vsl_conversion','lead_capture_launch','offer_validation'] THEN true
  WHEN user_objectives @> ARRAY['educational'] THEN false
  ELSE NULL
END
WHERE is_icp_profile IS NULL;

-- 3) Recriar view (DROP+CREATE para reordenar colunas)
...
SELECT p.id AS user_id,
   ...
   FROM profiles p
     ...
     LEFT JOIN ( SELECT quizzes.user_id,
            count(*) AS quiz_count,
            count(*) FILTER (WHERE quizzes.status = 'active'::quiz_status) AS active_quiz_count
           FROM quizzes
          GROUP BY quizzes.user_id) qz ON qz.user_id = p.id
     LEFT JOIN ( SELECT q.user_id,
          ...
          GROUP BY q.user_id) qr ON qr.user_id = p.id
