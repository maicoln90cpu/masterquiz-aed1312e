  ...
  try {
    ...
    if (usage) {
      ...
      await supabase.from('video_usage').insert({
        ...
        total_size_mb: video.size_mb, video_count: 1,
      });
    }
    ...
    return okResponse({ videoId, cdnUrl: video.cdn_url, status: 'ready' }, traceId, corsHeaders);
  } catch (e) {
    const message = e instanceof Error ? e.message : 'Unknown error';
    console.error('[bunny-chunked-complete]', message);
