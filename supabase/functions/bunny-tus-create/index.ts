...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { fileName, fileSize, fileType, quizId }: CreateRequest = await req.json();
    ...
    console.log(`TUS create request: ${fileName}, size: ${fileSize}, user: ${user.id}`);
    ...
    const tusUploadUrl = `https://storage.bunnycdn.com/${storageZoneName}/${uniqueFileName}`;

    console.log(`TUS upload URL created: ${tusUploadUrl}`);
    ...
    console.error('TUS create error:', error);
