...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!bunnyApiKey || !storageZoneName || !storageZonePassword || !cdnHostname) {
    ...
    const { fileName, fileSize, quizId }: UploadRequest = await req.json();
    ...
    const { data: videoRecord, error: insertError } = await supabase
      ...
      .insert({
        ...
        size_mb: fileSizeMb,
        status: 'uploading',
        cdn_url: `https://${cdnHostname}/${uniqueFileName}`
      })
      .select()
      .single();
    ...
    return new Response(
      JSON.stringify({
        error: 'This endpoint is deprecated. Use bunny-upload-video-multipart instead.',
        deprecated: true
      }),
