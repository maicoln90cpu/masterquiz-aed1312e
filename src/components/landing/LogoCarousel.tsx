...
export const LogoCarousel = () => {
  ...
  useEffect(() => {
    ...
    const step = () => {
      ...
      if (firstList) {
        const listWidth = firstList.scrollWidth + 100; // 100 = gap
  ...
  const LogoList = ({ ariaHidden }: { ariaHidden: boolean }) => (
    <ul className="flex items-center" role="list" aria-hidden={ariaHidden}>
      {LOGOS.map((logo) => (
        <li key={logo.alt} className="flex-none mr-[100px]" role="listitem">
          <img
            className="max-h-14 h-auto w-auto block pointer-events-none"
            src={logo.src}
            alt={logo.alt}
            loading="lazy"
            decoding="async"
